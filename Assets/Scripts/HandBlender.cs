using UnityEngine;

public class HandBlender : MonoBehaviour
{
    [SerializeField] private string displayText = "Press 'E' to use the hand blender"; // Text to display
    [SerializeField] private float offsetY = 20f; // Offset from the bottom of the screen
    [SerializeField] private float textHeight = 30f; // Height of the text box
    [SerializeField] private GUISkin customSkin; // Custom GUI skin for styling
    [SerializeField] private AudioClip blendingSound; // Sound effect for blending
    [SerializeField] private ChecklistManager checklistManager; // Reference to the ChecklistManager script

    private bool playerInRange; // Flag to track if the player is in the trigger area
    private bool isBlending; // Flag to track if the hand blender is blending
    private float blendTime = 5f; // Time it takes to blend (adjusted to 10 seconds)
    private float blendTimer; // Timer for blending process
    private AudioSource audioSource; // Reference to the AudioSource component

    // Start is called before the first frame update
    void Start()
    {
        // Get the AudioSource component attached to the same GameObject
        audioSource = GetComponent<AudioSource>();
    }

    // Update is called once per frame
    void Update()
    {
        if (playerInRange && Input.GetKeyDown(KeyCode.E) && !isBlending) // If player is in range, E key is pressed, and hand blender is not blending
        {
            StartBlending(); // Start blending with the hand blender
        }

        if (isBlending)
        {
            // Update the blending timer
            blendTimer -= Time.deltaTime;

            if (blendTimer <= 0f)
            {
                // Blending process completed
                FinishBlending();
            }
        }
    }

    // Method to start blending with the hand blender
    private void StartBlending()
    {
        isBlending = true;
        blendTimer = blendTime;
        Debug.Log("Hand blender blending started!");

        // Play the blending sound effect
        if (audioSource != null && blendingSound != null)
        {
            audioSource.PlayOneShot(blendingSound);
        }

    }

    // Method to finish blending with the hand blender
    private void FinishBlending()
    {
        isBlending = false;
        Debug.Log("Hand blender blending finished!");

        // Destroy the hand blender object
        Destroy(gameObject);

        // Add the hand blender to the checklist
        checklistManager.ObjectPickedUp(gameObject);
    }

    // Called when another collider enters the trigger area
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Player")) // Check if the entering collider is the player
        {
            playerInRange = true; // Set playerInRange flag to true
        }
    }

    // Called when another collider exits the trigger area
    private void OnTriggerExit(Collider other)
    {
        if (other.CompareTag("Player")) // Check if the exiting collider is the player
        {
            playerInRange = false; // Set playerInRange flag to false
        }
    }

    // Draw GUI elements
    private void OnGUI()
    {
        if (!playerInRange || isBlending) // If the player is not in range or the hand blender is blending, do not display the text
            return;

        if (customSkin != null) // Apply custom GUI skin if available
        {
            GUI.skin = customSkin;
        }

        float screenWidth = Screen.width;
        float screenHeight = Screen.height;

        float boxWidth = screenWidth * 0.5f; // Width of the text box
        float boxX = (screenWidth - boxWidth) * 0.5f; // X position of the text box
        float boxY = screenHeight - offsetY - textHeight; // Y position of the text box

        // Draw the GUI box with the displayText at the specified position and size
        GUI.Box(new Rect(boxX, boxY, boxWidth, textHeight), displayText);
    }
}
